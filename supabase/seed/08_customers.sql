...
-- A delivery partner, used by the delivery app's local assignment and proof flows.
insert into auth.users (
  instance_id, id, aud, role, email, encrypted_password, email_confirmed_at,
  raw_app_meta_data, raw_user_meta_data, created_at, updated_at,
  confirmation_token, recovery_token, email_change_token_new, email_change_token_current,
...
select '00000000-0000-0000-0000-000000000000'::uuid,
       '66666666-6666-4666-8666-666666666666'::uuid, 'authenticated', 'authenticated',
       'delivery.agent@example.novamart.in',
       extensions.crypt('NovaMart#Local1', extensions.gen_salt('bf')),
       ...
       '', '', '', '', '', '', '', ''
 where not exists (select 1 from auth.users u where u.id = '66666666-6666-4666-8666-666666666666'::uuid);
...
select u.id::text, u.id,
       ...
       'email', now(), now()
  from auth.users u
 where u.id = '66666666-6666-4666-8666-666666666666'::uuid
   and not exists (select 1 from auth.identities i where i.user_id = u.id and i.provider = 'email');

insert into identity.user_roles (user_id, role_id, grant_reason)
select '66666666-6666-4666-8666-666666666666'::uuid, r.id, 'Seed: local delivery partner'
  from identity.roles r
 where r.code = 'DELIVERY_AGENT'
   and exists (select 1 from identity.profiles p where p.id = '66666666-6666-4666-8666-666666666666'::uuid)
on conflict do nothing;
