...
-- An active assigned shipment gives delivery and warehouse clients a truthful local
-- work queue without touching the delivered order above.
do $$
declare
  v_user_id uuid := '22222222-2222-4222-8222-222222222222';
  v_agent_id uuid := '66666666-6666-4666-8666-666666666666';
  v_order_id uuid;
  v_item_id uuid;
  v_sku_id uuid;
  v_product_id uuid;
...
begin
  ...
  select l.id, l.seller_id, l.sku_id, l.product_id, l.default_warehouse_id
    ...
    join catalog.skus sk on sk.id = l.sku_id
    join seller.sellers s on s.id = l.seller_id
   where s.slug = 'aurex-official-store' and sk.sku_code = 'NM-VOLT-SRG-BLU-128'
   limit 1;
  ...
  insert into commerce.orders (
    ...
    total_payable_paise, amount_paid_paise, payment_method, payment_status,
    is_cod, delivery_pincode, promised_delivery_date, client_platform,
    client_version, placed_at, confirmed_at
  ) values (
    'NM100000902', v_user_id, 'OUT_FOR_DELIVERY', 'SHIPPED', 'INR',
    1, 1, 1, v_price, 0, 0, 0, v_tax, v_price, v_price, 'UPI', 'PAID',
    false, '400050', current_date, 'android', '1.0.0',
  ...
  select v_order_id, 1, v_listing_id, v_sku_id, v_product_id, v_seller_id, v_warehouse_id,
         p.title, pv.variant_label, sk.sku_code, b.name,
         (select pm.public_url from catalog.product_media pm where pm.product_id = p.id and pm.is_primary limit 1),
         p.hsn_code, 1, 'OUT_FOR_DELIVERY', 'SELLER_FULFILLED', 7, 'REFUND_OR_REPLACEMENT',
         current_date + 7, true, current_date,
         now() - interval '1 day'
    from catalog.products p
    join catalog.product_variants pv on pv.product_id = p.id and pv.is_default
    join catalog.skus sk on sk.id = v_sku_id and sk.variant_id = pv.id
  ...
  insert into commerce.order_item_price_breakdowns (
    ...
    v_item_id, v_order_id, 1, 1699900, v_price, v_price, 0,
    124576, 18.000, 11212, 11212, v_tax, true, 'MH', v_price, 8.000,
    11760, 2117, 133123, '[{"source":"seed","rule":"active_delivery_fixture"}]'::jsonb
  );
  ...
  insert into commerce.order_price_breakdowns (
    ...
    v_order_id, v_price, 0, 124576, 11212, 11212, v_tax, v_price,
    '[{"source":"seed","rule":"active_delivery_fixture"}]'::jsonb
  );
  ...
  insert into payments.payment_intents (
    ...
    v_order_id, v_user_id, 'MOCK', 'mock_seed_nm100000902', v_price, v_price,
    'UPI', 'CAPTURED', now() - interval '2 days', now() - interval '2 days',
    'seed:NM100000902'
  );

  insert into fulfillment.shipments (
    shipment_reference, order_id, seller_id, warehouse_id, carrier_id,
    awb_number, provider_shipment_id, status, delivery_agent_id,
    declared_value_paise, actual_weight_grams, pickup_pincode, delivery_pincode,
    zone_code, delivery_address, pickup_address, promised_delivery_date,
    estimated_delivery_date, picked_up_at
  ) values (
    'SH100000902', v_order_id, v_seller_id, v_warehouse_id, v_carrier_id,
    'MOCKNM100000902', 'mock-shipment-seed-902', 'OUT_FOR_DELIVERY', v_agent_id,
    v_price, 460, '560034', '400050', 'ZONAL',
    '{"recipient_name":"Rahul Mehta","recipient_phone":"919000000002","city":"Mumbai","pincode":"400050"}'::jsonb,
    '{"warehouse":"Aurex Delhi","pincode":"560034"}'::jsonb,
    current_date, current_date, now() - interval '1 day'
  ) returning id into v_shipment_id;
  ...
  insert into fulfillment.tracking_events (
    shipment_id, provider_event_id, carrier_status_code, normalised_status,
    description, occurred_at, location, raw_payload, was_applied
  ) values (
    v_shipment_id, 'mock-seed-902-ofd', 'OUT_FOR_DELIVERY', 'OUT_FOR_DELIVERY',
    'Delivery partner is on the way', now() - interval '2 hours', 'Mumbai',
    '{"source":"seed"}'::jsonb, true
  );

  insert into fulfillment.delivery_agent_shifts (delivery_agent_id, warehouse_id, shift_date, status, started_at, shipments_assigned)
  values (v_agent_id, v_warehouse_id, current_date, 'ON_DUTY', now() - interval '4 hours', 1)
  on conflict (delivery_agent_id, shift_date) do nothing;
end;
$$;
